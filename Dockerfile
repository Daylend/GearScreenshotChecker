...
RUN apt update && DEBIAN_FRONTEND=noninteractive apt install --no-install-recommends wget -y && \
    ...
    wget --no-check-certificate -O /tmp/tf_nightly-1.13.1-cp36-none-any.whl https://github.com/Tzeny/tensorflowbuilds/raw/master/tensorflow-1.13.1-cp36-cp36m-linux_x86_64.whl && \
